...
public sealed class BlinkingSystem : EntitySystem
{
    ...
    public float GetBlinkUrgency(BlinkingComponent comp)
    {
        ...
        var timeSinceBlink = (float)(_timing.CurTime - comp.LastBlinkTime).TotalSeconds;
        var blurRange = comp.MaxTimeWithoutBlink - comp.BlurStartTime;

        if (timeSinceBlink < comp.BlurStartTime || blurRange <= 0f)
            return 0f;

        return Math.Clamp((timeSinceBlink - comp.BlurStartTime) / blurRange, 0f, 1f);
    }
